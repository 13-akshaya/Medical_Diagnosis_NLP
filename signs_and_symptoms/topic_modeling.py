...
target = ['Cardiovascular', 'Eye','ENT', 'Skin',
          'Genetic', 'ObGyn', 'Respiratory',
...
# Create lda model
lda = models.LdaModel(corpus=corpus, num_topics=9, id2word=id2word, passes=5)
lda.print_topics(9)

# PERFORMING COREX
words = list(numpy.asarray(count_vectorizer.get_feature_names_out()))
topic_model = corextopic.Corex(n_hidden=9, words=words, seed=1)
...
topic_word = pandas.DataFrame(lsa.components_.round(9),
             index = ['component'+str(i) for i in range(9)],
...
result1 = display_topics(lsa, count_vectorizer.get_feature_names_out(), 9)

#tem_list
final_dic = {}
final_dic["Cardiovascular"] = tem_list[0]
final_dic["Eye"] = tem_list[1]
final_dic["ENT"] = tem_list[2]
final_dic["Skin"] = tem_list[3]
final_dic["Genetic"] = tem_list[4]
final_dic["ObGyn"] = tem_list[5]
final_dic["Respiratory"] = tem_list[6]
final_dic["Neurologic"] = tem_list[7]
final_dic["Gastrointestinal"] = tem_list[8]
...
# Declare a list that is to be converted into a column
d_name = ['Cardiovascular', 'Eye','ENT', 'Skin',
          'Genetic', 'ObGyn', 'Respiratory',
          'Neurologic', 'Gastrointestinal']
